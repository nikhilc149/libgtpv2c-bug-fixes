      ...
      detach_ack_t *value)
{
    ...
    if (value->header.gtpc.teid_flag)
      ...
      return count + ((value->header.gtpc.teid_flag == 1) ? sizeof(gtpv2c_header_t) : (sizeof(gtpv2c_header_t) - 4));
}
/**
    ...
    if (value->header.gtpc.teid_flag)
      ...
      count = 0;
          ...
          if (ie_header->type == GTP_IE_CAUSE && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            ...
            //count += decode_gtp_bearer_context_ie(buf + count, &value->bearer_contexts[value->bearer_count++]);
			count += decode_gtp_create_indir_data_fwdng_tunn_response__bearer_ctxt_ie(buf + count,
										&value->bearer_contexts[value->bearer_count++]);
      }  else if (ie_header->type == GTP_IE_RECOVERY && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            count += decode_gtp_recovery_ie(buf + count, &value->recovery);
      }  else if (ie_header->type == GTP_IE_PRIV_EXT) {
            count += decode_gtp_priv_ext_ie(buf + count, &value->priv_ext);
...
/**
* Decodes del_indir_data_fwdng_tunn_rsp_t to buffer.
* @param buf
*   buffer to store decoded values.
* @param value
*  del_indir_data_fwdng_tunn_rsp_t
*     * @return
*   number of decoded bytes.
*/
int decode_del_indir_data_fwdng_tunn_rsp(uint8_t *buf,
		del_indir_data_fwdng_tunn_resp_t *value)
{
	uint16_t count = 0;
	uint16_t buf_len = 0;
    ...
    if (value->header.gtpc.teid_flag)
      ...
      count = 0;
          ...
          if (ie_header->type == GTP_IE_AGG_MAX_BIT_RATE && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            ...
            count += decode_gtp_mod_bearer_command__bearer_ctxt_ie(buf + count, &value->bearer_context);
      }  else if (ie_header->type == GTP_IE_OVRLD_CTL_INFO && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            count += decode_gtp_ovrld_ctl_info_ie(buf + count, &value->mmes4_sgsns_ovrld_ctl_info);
      }  else if (ie_header->type == GTP_IE_OVRLD_CTL_INFO && ie_header->instance == GTP_IE_INSTANCE_ONE) {
            count += decode_gtp_ovrld_ctl_info_ie(buf + count, &value->sgws_ovrld_ctl_info);
		...
		mod_acc_bearers_rsp_t *value)
{
	uint16_t count = 0;
	uint16_t buf_len = 0;
	value->bearer_modify_count = 0;
	value->bearer_remove_count = 0;
	...
	while (count < buf_len) {
		...
		if (ie_header->type == GTP_IE_CAUSE && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			...
			count += decode_gtp_mod_bearer_response_bearer_ctxt_modified_ie(buf + count,
					&value->bearer_contexts_modified[value->bearer_modify_count]);
			value->bearer_modify_count++;
		}  else if (ie_header->type == GTP_IE_MOD_BEARER_REQUEST_BEARER_CTXT_TO_BE_REMOVED && ie_header->instance == GTP_IE_INSTANCE_ONE) {
			 count += decode_gtp_mod_bearer_response_bearer_ctxt_marked_removal_ie(buf + count,
					 &value->bearer_contexts_marked_removal[value->bearer_remove_count]);
			 value->bearer_remove_count++;
		}  else if (ie_header->type == GTP_IE_RECOVERY && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			count += decode_gtp_recovery_ie(buf + count, &value->recovery);
		}  else if (ie_header->type == GTP_IE_INDICATION && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			count += decode_gtp_indication_ie(buf + count, &value->indctn_flgs);
      ...
      mod_acc_bearers_req_t *value)
{
	...
	while (count < buf_len) {
		...
		if (ie_header->type == GTP_IE_INDICATION && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			...
			count += decode_gtp_mod_bearer_request_bearer_ctxt_to_be_modified_ie(buf + count,
					&value->bearer_contexts_to_be_modified[value->bearer_modify_count]);
			value->bearer_modify_count++;
		}  else if (ie_header->type == GTP_IE_MOD_BEARER_REQUEST_BEARER_CTXT_TO_BE_REMOVED && ie_header->instance == GTP_IE_INSTANCE_ONE) {
			count += decode_gtp_mod_bearer_request_bearer_ctxt_to_be_removed_ie(buf + count,
					&value->bearer_contexts_to_be_removed[value->bearer_remove_count]);
			value->bearer_remove_count++;
		}  else if (ie_header->type == GTP_IE_RECOVERY && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			count += decode_gtp_recovery_ie(buf + count, &value->recovery);
		}  else if (ie_header->type == GTP_IE_SECDRY_RAT_USAGE_DATA_RPT && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			count += decode_gtp_secdry_rat_usage_data_rpt_ie(buf + count,
					&value->secdry_rat_usage_data_rpt[value->second_rat_count]);
			value->second_rat_count++;
		}  else if (ie_header->type == GTP_IE_PRIV_EXT) {
			count += decode_gtp_priv_ext_ie(buf + count, &value->priv_ext);
		}  else
    ...
    if (value->header.gtpc.teid_flag)
      ...
      count = 0;
          ...
          if (ie_header->type == GTP_IE_IMSI && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            ...
            count += decode_gtp_create_indir_data_fwdng_tunn_request__bearer_ctxt_ie(buf + count, &value->bearer_contexts[value->bearer_count++]);
      }  else if (ie_header->type == GTP_IE_RECOVERY && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
            count += decode_gtp_recovery_ie(buf + count, &value->recovery);
      }  else if (ie_header->type == GTP_IE_PRIV_EXT) {
            count += decode_gtp_priv_ext_ie(buf + count, &value->priv_ext);
...
/*
* Decodes del_indir_data_fwdng_tunn_req_t to buffer.
* @param buf
*   buffer to store decoded values.
* @param value
...
*   number of decoded bytes.
*/
int decode_del_indir_data_fwdng_tunn_req(uint8_t *buf,
		del_indir_data_fwdng_tunn_req_t *value)
{
	uint16_t count = 0;
	count += decode_gtpv2c_header_t(buf + count, &value->header);
	//return count;
...
/**
 * Decodes gtp_resume_notification_t to buffer.
 * @param buf
 *   buffer to store decoded values.
 * @param value
...
int decode_resume_notification(uint8_t *buf, gtp_resume_notification_t *value)
{
	...
	while (count < buf_len) {
		...
		if (ie_header->type == GTP_IE_IMSI && ie_header->instance == GTP_IE_INSTANCE_ZERO) {
			...
			count += decode_gtp_fully_qual_tunn_endpt_idnt_ie(buf + count, &value->sender_fteid);
		} else if (ie_header->type == GTP_IE_PRIV_EXT) {
			count += decode_gtp_priv_ext_ie(buf + count, &value->priv_ext);
		} else {
			count += sizeof(ie_header_t) + ntohs(ie_header->len);
...
/**
 * Decodes gtp_resume_acknowledge_t to buffer.
 * @param buf
 *   buffer to store decoded values.
 * @param value
...
int decode_resume_acknowledge(uint8_t *buf, gtp_resume_acknowledge_t *value)
{
...
int decode_suspend_notification(uint8_t *buf, gtp_suspend_notification_t *value)
{
	uint16_t count = 0;
	uint16_t buf_len = 0;
...
int decode_suspend_acknowledge(uint8_t *buf, gtp_suspend_acknowledgement_t *value)
{
