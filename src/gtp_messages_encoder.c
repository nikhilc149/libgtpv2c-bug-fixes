...
for(uint8_t iterator = 0; iterator < MAX_BEARERS; iterator++) {
	if (value->bearer_contexts[iterator].header.len) {
		encoded += encode_gtp_create_indir_data_fwdng_tunn_response__bearer_ctxt_ie(
									&(value->bearer_contexts[iterator]), buf + encoded);
	}
}
...
/**
* Encodes del_indir_data_fwdng_tunn_rsp_t to buffer.
* @param buf
*   buffer to store encoded values.
* @param value
...
*   number of encoded bytes.
*/
int encode_del_indir_data_fwdng_tunn_rsp(del_indir_data_fwdng_tunn_resp_t *value,
		uint8_t *buf)
{
	uint16_t encoded = 0;
	encoded += encode_gtpv2c_header_t(&value->header, buf +encoded);
...
if (value->bearer_context.header.len)
        encoded += encode_gtp_mod_bearer_command__bearer_ctxt_ie(&(value->bearer_context), buf + encoded);
    ...
    uint8_t *buf)
{
	...
	for(uint8_t i = 0; i< MAX_BEARERS; i++) {
		if (value->bearer_contexts_modified[i].header.len)
			encoded += encode_gtp_mod_bearer_response_bearer_ctxt_modified_ie(&(value->bearer_contexts_modified[i]), buf + encoded);
	}

	for(uint8_t i = 0; i< MAX_BEARERS; i++) {
		if (value->bearer_contexts_marked_removal[i].header.len)
			encoded += encode_gtp_mod_bearer_response_bearer_ctxt_marked_removal_ie(&(value->bearer_contexts_marked_removal[i]), buf + encoded);
	}
		...
		uint8_t *buf)
{
	...
	for(uint8_t i = 0; i< MAX_BEARERS ;i++) {
		if (value->bearer_contexts_to_be_modified[i].header.len)
			encoded +=encode_gtp_mod_bearer_request_bearer_ctxt_to_be_modified_ie(&(value->bearer_contexts_to_be_modified[i]), buf + encoded);
	}
	for(uint8_t i = 0; i< MAX_BEARERS ;i++) {
		if (value->bearer_contexts_to_be_removed[i].header.len)
			encoded += encode_gtp_mod_bearer_request_bearer_ctxt_to_be_removed_ie(&(value->bearer_contexts_to_be_removed[i]), buf + encoded);
	}
...
for(uint8_t iterator = 0; iterator < value->bearer_count; iterator++) {
		if (value->bearer_contexts[iterator].header.len) {
			encoded += encode_gtp_create_indir_data_fwdng_tunn_request__bearer_ctxt_ie(
					&(value->bearer_contexts[iterator]), buf + encoded);
		}
	}
if (value->recovery.header.len)
...
/**
* Encodes del_indir_data_fwdng_tunn_req_t to buffer.
* @param buf
*   buffer to store encoded values.
* @param value
...
*   number of encoded bytes.
*/
int encode_del_indir_data_fwdng_tunn_req(del_indir_data_fwdng_tunn_req_t *value,
    uint8_t *buf)
{
    uint16_t encoded = 0;
    encoded += encode_gtpv2c_header_t(&value->header, buf +encoded);
...
/**
* Encodes gtp_resume_notification_t to buffer.
* @param buf
*   buffer to store encoded values.
* @param value
...
*   number of encoded bytes.
*/
int encode_resume_notification(gtp_resume_notification_t *value,
		uint8_t *buf)
{
	...
	if (value->sender_fteid.header.len)
		encoded += encode_gtp_fully_qual_tunn_endpt_idnt_ie(&(value->sender_fteid), buf + encoded);
...
/**
* Encodes gtp_resume_acknowledge_t to buffer.
* @param buf
*   buffer to store encoded values.
* @param value
...
*   number of encoded bytes.
*/
int encode_resume_acknowledge(gtp_resume_acknowledge_t *value,
		uint8_t *buf)
{
...
int encode_suspend_notification(gtp_suspend_notification_t *value,
		uint8_t *buf)
{
...
int encode_suspend_acknowledge(gtp_suspend_acknowledgement_t *value,
		uint8_t *buf)
{
