...
/**
* Encodes home_enb_id_fld_t to buffer.
* @param buf
*   buffer to store encoded values.
* @param value
    macro_enb_id_fld_t
...
*   number of encoded bytes.
*/
int encode_home_enb_id_fld(const home_enb_id_fld_t *value,
    uint8_t *buf)
{
    uint16_t encoded = 0;
    encoded += encode_bits(value->henbid_mcc_digit_2, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_mcc_digit_1, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_mnc_digit_3, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_mcc_digit_3, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_mnc_digit_2, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_mnc_digit_1, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_spare, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_home_enodeb_id, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->henbid_home_enb_id2, 24, buf + (encoded/8), encoded % CHAR_SIZE);
    ...
    uint8_t *buf)
{
    ...
    encoded += encode_bits(value->pco.pco_iei, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->pco.pco_content_len, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->pco.pco_flag_ext, 1, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->pco.pco_flag_spare, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->pco.pco_cfg_proto, 3, buf + (encoded/8), encoded % CHAR_SIZE);

    for(int i = 0; i < value->pco.pco_content_count; i++) {
    	encoded += encode_bits(value->pco.pco_content[i].prot_id, 16, buf + (encoded/8), encoded % CHAR_SIZE);
    	encoded += encode_bits(value->pco.pco_content[i].length, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    	memcpy(buf + (encoded/8), value->pco.pco_content[i].content, value->pco.pco_content[i].length);
    ...
    uint8_t *buf)
{
    ...
    memcpy(buf + (encoded/8), &value->hsgw_addr_fwdng, value->hsgw_addr_fwdng_len);
    encoded += value->hsgw_addr_fwdng_len * CHAR_SIZE;
    ...
    encoded += encode_bits(value->spare2, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->eps_bearer_id, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    ...
    uint8_t *buf)
{
    ...
    memcpy(buf + (encoded/CHAR_SIZE), &value->sgw_address, value->sgw_addr_len);
    encoded += value->sgw_addr_len * CHAR_SIZE;
    ...
    uint8_t *buf)
{
    ...
    if(value->number_of_tai > 0){
        for(int i = 0; i < value->number_of_tai; i++)
            encoded += encode_tai_field(&(value->tais[i]), buf + encoded);
    }
    if(value->nbr_of_macro_enb > 0){
        for(int i = 0; i < value->nbr_of_macro_enb; i++)
            encoded += encode_macro_enb_id_fld(&(value->macro_enb_ids[i]), buf + encoded);
    }
    if(value->nbr_of_home_enb > 0){
        for(int i = 0; i < value->nbr_of_home_enb; i++)
            encoded += encode_home_enb_id_fld(&(value->home_enb_ids[i]), buf + encoded);
    }
    if(value->number_of_ecgi > 0){
        for(int i = 0; i < value->number_of_ecgi; i++)
            encoded += encode_ecgi_field(&(value->ecgis[i]), buf + encoded);
    }
    if(value->number_of_rai > 0){
        for(int i = 0; i < value->number_of_rai; i++)
            encoded += encode_rai_field(&(value->rais[i]), buf + encoded);
    }
    if(value->number_of_sai > 0){
        for(int i = 0; i < value->number_of_sai; i++)
            encoded += encode_sai_field(&(value->sais[i]), buf + encoded);
    }
    if(value->number_of_cgi){
        for(int i = 0; i < value->number_of_cgi; i++)
            encoded += encode_cgi_field(&(value->cgis[i]), buf + encoded);
    }
    encoded = encoded*CHAR_SIZE;
    ...
    if(value->nbr_of_extnded_macro_enb > 0){
        for(int i = 0; i < value->nbr_of_extnded_macro_enb; i++)
            encoded += encode_extnded_macro_enb_id_fld(&(value->extnded_macro_enb_ids[i]), buf + encoded);
    }
    ...
    uint8_t *buf)
{
    ...
    encoded += encode_bits(value->ip_addr_of_trc_coll_entity, 32, buf + (encoded/8), encoded % CHAR_SIZE);
    ...
    uint8_t *buf)
{
    ...
    if (value->pdn_type == PDN_IP_TYPE_IPV6 || value->pdn_type == PDN_IP_TYPE_IPV4V6) {

        encoded += encode_bits(value->ipv6_prefix_len, 8, buf + (encoded/8), encoded % CHAR_SIZE);

        memcpy(buf + (encoded/8), &value->paa_ipv6, IPV6_ADDRESS_LEN);
        encoded += IPV6_ADDRESS_LEN * CHAR_SIZE;

    }

    if (value->pdn_type == PDN_IP_TYPE_IPV4 || value->pdn_type == PDN_IP_TYPE_IPV4V6) {
	    memcpy(buf + (encoded/8), &value->pdn_addr_and_pfx, 4);
        encoded += 4 * CHAR_SIZE;
    }
    ...
    uint8_t *buf)
{
    uint16_t encoded = 0;
    encoded += encode_ie_header_t(&value->header, buf + (encoded/CHAR_SIZE));
    encoded += encode_bits(value->int_nbr_val, 32, buf + (encoded/8), encoded % CHAR_SIZE);
    ...
    uint8_t *buf)
{
	...
	if (value->node_id_type == GTP_IPV4_GLOBAL_UNICAST) {
		memcpy(buf + (encoded/8), &value->node_address, 4);
		encoded += 4 * CHAR_SIZE;
	} else {
		memcpy(buf + (encoded/8), &value->node_address, 16);
    ...
    uint8_t *buf)
{
    ...
    encoded += encode_bits(value->apco.pco_iei, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->apco.pco_content_len, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->apco.pco_flag_ext, 1, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->apco.pco_flag_spare, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->apco.pco_cfg_proto, 3, buf + (encoded/8), encoded % CHAR_SIZE);

    for(int i = 0; i < value->apco.pco_content_count; i++) {
    	encoded += encode_bits(value->apco.pco_content[i].prot_id, 16, buf + (encoded/8), encoded % CHAR_SIZE);
    	encoded += encode_bits(value->apco.pco_content[i].length, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    	memcpy(buf + (encoded/8), value->apco.pco_content[i].content, value->apco.pco_content[i].length);
    	encoded += value->apco.pco_content[i].length * CHAR_SIZE;
    }
    ...
    uint8_t *buf)
{
    ...
    encoded += encode_bits(value->epco.pco_iei, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->epco.pco_content_len, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->epco.pco_flag_ext, 1, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->epco.pco_flag_spare, 4, buf + (encoded/8), encoded % CHAR_SIZE);
    encoded += encode_bits(value->epco.pco_cfg_proto, 3, buf + (encoded/8), encoded % CHAR_SIZE);

    for(int i = 0; i < value->epco.pco_content_count; i++) {
    	encoded += encode_bits(value->epco.pco_content[i].prot_id, 16, buf + (encoded/8), encoded % CHAR_SIZE);
    	encoded += encode_bits(value->epco.pco_content[i].length, 8, buf + (encoded/8), encoded % CHAR_SIZE);
    	memcpy(buf + (encoded/8), value->epco.pco_content[i].content, value->epco.pco_content[i].length);
    	encoded += value->epco.pco_content[i].length * CHAR_SIZE;
    }
    ...
    uint8_t *buf)
{
    ...
    encoded += encode_ie_header_t(&value->header, buf + (encoded/CHAR_SIZE));
	/*
	 * As per the spec : 29274 : The Length field may have only two values
	 * (4 or 16) that determine if the Value field contains IPv4 or IPv6 address.
	 * */
	memcpy(buf + (encoded/8), &value->ipv4_ipv6_addr, value->header.len);
	encoded += value->header.len * CHAR_SIZE;
