...
/**
* decodes home_enb_id_fld_t to buffer.
* @param buf
* buffer to store decoded values.
* @param value
    home_enb_id_fld_t
...
*   number of decoded bytes.
*/
int decode_home_enb_id_fld(uint8_t *buf,
        home_enb_id_fld_t *value)
{
    uint16_t total_decoded = 0;
    uint16_t decoded = 0;
    value->henbid_mcc_digit_2 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_mcc_digit_1 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_mnc_digit_3 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_mcc_digit_3 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_mnc_digit_2 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_mnc_digit_1 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_spare = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_home_enodeb_id = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->henbid_home_enb_id2 = decode_bits(buf, total_decoded, 24, &decoded);
    total_decoded += decoded;
    return total_decoded/CHAR_SIZE;
}
        ...
        gtp_prot_cfg_opts_ie_t *value)
{
    ...
    for(int itr = 0; itr < value->pco.pco_content_count; itr++) {
    	value->pco.pco_content[itr].prot_id = decode_bits(buf, total_decoded, 16, &decoded);
    	total_decoded += decoded;
    	value->pco.pco_content[itr].length = decode_bits(buf, total_decoded, 8, &decoded);
    	total_decoded += decoded;
    	memcpy(value->pco.pco_content[itr].content, buf + (total_decoded/8), value->pco.pco_content[itr].length);
    	total_decoded += value->pco.pco_content[itr].length * 8;
    }
        ...
        gtp_s103_pdn_data_fwdng_info_ie_t *value)
{
    ...
    memcpy((char *)value->hsgw_addr_fwdng, buf + (total_decoded/CHAR_SIZE), value->hsgw_addr_fwdng_len);
    total_decoded += value->hsgw_addr_fwdng_len * CHAR_SIZE;
    ...
    value->spare2 = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->eps_bearer_id = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
        ...
        gtp_s1u_data_fwdng_ie_t *value)
{
    ...
    memcpy(value->sgw_address, buf + (total_decoded/CHAR_SIZE), value->sgw_addr_len);
    total_decoded += value->sgw_addr_len * CHAR_SIZE;
        ...
        gtp_pres_rptng_area_act_ie_t *value)
{
    ...
    if(value->number_of_tai > 0){
        for(int i = 0; i < value->number_of_tai; i++)
            total_decoded += decode_tai_field(buf + total_decoded, &value->tais[i]);
    }
    if(value->nbr_of_macro_enb > 0){
        for(int i = 0; i < value->nbr_of_macro_enb; i++)
            total_decoded += decode_macro_enb_id_fld(buf + total_decoded, &value->macro_enb_ids[i]);
    }
    if(value->nbr_of_home_enb > 0){
        for(int i = 0; i < value->nbr_of_home_enb; i++)
            total_decoded += decode_home_enb_id_fld(buf + total_decoded, &value->home_enb_ids[i]);
    }
    if(value->number_of_ecgi > 0){
        for(int i = 0; i < value->number_of_ecgi; i++)
            total_decoded += decode_ecgi_field(buf + total_decoded, &value->ecgis[i]);
    }
    if(value->number_of_rai > 0){
        for(int i = 0; i < value->number_of_rai; i++)
            total_decoded += decode_rai_field(buf + total_decoded, &value->rais[i]);
    }
    ...
    if( value->number_of_cgi > 0){
        for(int i = 0; i < value->number_of_cgi; i++)
            total_decoded += decode_cgi_field(buf + total_decoded, &value->cgis[i]);
    }
    total_decoded = total_decoded*CHAR_SIZE;
        ...
        gtp_trc_info_ie_t *value)
{
    ...
    value->ip_addr_of_trc_coll_entity = decode_bits(buf, total_decoded, 32, &decoded);
    total_decoded += decoded;
    return value->header.len + IE_HEADER_SIZE;
}
        ...
        gtp_pdn_addr_alloc_ie_t *value)
{
    ...
    if (value->pdn_type == PDN_IP_TYPE_IPV6 || value->pdn_type == PDN_IP_TYPE_IPV4V6) {
        ...
        value->ipv6_prefix_len = decode_bits(buf, total_decoded, 8, &decoded);
        total_decoded += decoded;

        memcpy(&value->paa_ipv6, buf + (total_decoded/CHAR_SIZE), IPV6_ADDRESS_LEN);
        total_decoded +=  IPV6_ADDRESS_LEN * CHAR_SIZE;
    }

    if (value->pdn_type == PDN_IP_TYPE_IPV4 || value->pdn_type == PDN_IP_TYPE_IPV4V6) {
        memcpy(&value->pdn_addr_and_pfx, buf + (total_decoded/CHAR_SIZE), 4);
	    total_decoded += 4 * CHAR_SIZE;
    }
        ...
        gtp_integer_number_ie_t *value)
{
    ...
    total_decoded += decode_ie_header_t(buf, &(value->header), IE_HEADER_SIZE);
    uint16_t decoded = 0;
    value->int_nbr_val = decode_bits(buf, total_decoded, value->header.len * CHAR_SIZE, &decoded);
    total_decoded += decoded;
    return value->header.len + IE_HEADER_SIZE;
}
        ...
        gtp_fqcsid_ie_t *value)
{
	...
	if (value->node_id_type == GTP_IPV4_GLOBAL_UNICAST) {
		memcpy(&value->node_address, buf + (total_decoded/CHAR_SIZE), 4);
		total_decoded += 4 * CHAR_SIZE;
	} else {
		memcpy(&value->node_address, buf + (total_decoded/CHAR_SIZE), 16);
		total_decoded += 16 * CHAR_SIZE;
	}
        ...
        gtp_addtl_prot_cfg_opts_ie_t *value)
{
	...
	value->apco.pco_flag_ext = decode_bits(buf, total_decoded, 1, &decoded);
	total_decoded += decoded;
	value->apco.pco_flag_spare = decode_bits(buf, total_decoded, 4, &decoded);
	total_decoded += decoded;
	value->apco.pco_cfg_proto = decode_bits(buf, total_decoded, 3, &decoded);
	total_decoded += decoded;

	for(int itr = 0; itr < value->apco.pco_content_count; itr++) {
		value->apco.pco_content[itr].prot_id = decode_bits(buf, total_decoded, 16, &decoded);
		total_decoded += decoded;
		value->apco.pco_content[itr].length = decode_bits(buf, total_decoded, 8, &decoded);
		total_decoded += decoded;
		memcpy(value->apco.pco_content[itr].content, buf + (total_decoded/8), value->apco.pco_content[itr].length);
		total_decoded += value->apco.pco_content[itr].length * 8;
	}
        ...
        gtp_extnded_prot_cfg_opts_ie_t *value)
{
    ...
    total_decoded += decode_ie_header_t(buf, &(value->header), IE_HEADER_SIZE);
    uint16_t decoded = 0;
    value->epco.pco_flag_ext = decode_bits(buf, total_decoded, 1, &decoded);
    total_decoded += decoded;
    value->epco.pco_flag_spare = decode_bits(buf, total_decoded, 4, &decoded);
    total_decoded += decoded;
    value->epco.pco_cfg_proto = decode_bits(buf, total_decoded, 3, &decoded);
    total_decoded += decoded;

    for(int itr = 0; itr < value->epco.pco_content_count; itr++) {
    	value->epco.pco_content[itr].prot_id = decode_bits(buf, total_decoded, 16, &decoded);
    	total_decoded += decoded;
    	value->epco.pco_content[itr].length = decode_bits(buf, total_decoded, 8, &decoded);
    	total_decoded += decoded;
    	memcpy(value->epco.pco_content[itr].content, buf + (total_decoded/8), value->epco.pco_content[itr].length);
    	total_decoded += value->epco.pco_content[itr].length * 8;
    }
        ...
        gtp_ip_address_ie_t *value)
{
	 ...
	 **/
	memcpy(value->ipv4_ipv6_addr, (buf + (total_decoded/8)), value->header.len);
    total_decoded += decoded;
    return value->header.len + IE_HEADER_SIZE;
}
