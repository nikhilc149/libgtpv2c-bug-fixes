...
#define GTP_PCO_CONTENT_LEN (255)
...
/* fq_csid_node_id_type */
#define GTP_IPV4_GLOBAL_UNICAST (0)
#define GTP_IPV6_GLOBAL_UNICAST (1)
#define GTP_MCC_MNC (2)

#define MAX_TAIS                (15)
#define MAX_MACRO_ENB_IDS       (63)
#define MAX_HOME_ENB_IDS        (63)
#define MAX_ECGIS               (63)
#define MAX_RAIS                (15)
#define MAX_SAIS                (63)
#define MAX_CGIS                (63)
#define MAX_EX_MACRO_ENB_IDS    (63)
...
typedef struct home_enb_id_fld_t {
...
typedef struct gtp_ip_address_ie_t {
  ie_header_t header;
  /* TODO: Need to check */
  uint8_t ipv4_ipv6_addr[IPV6_ADDRESS_LEN];
...
typedef struct prot_content_t {
	uint16_t prot_id;
	uint8_t length;
	uint8_t content[GTP_PCO_CONTENT_LEN];
} prot_content_t;

typedef struct prot_cfg_opts_t {
	uint8_t pco_iei;
	uint8_t pco_content_len;
	uint8_t pco_flag_ext :1;
	uint8_t pco_flag_spare :4;
	uint8_t pco_cfg_proto :3;
	uint8_t pco_content_count;
	prot_content_t pco_content[8];
} prot_cfg_opts_t;
...
typedef struct gtp_pdn_addr_alloc_ie_t {
  ...
  uint8_t paa_ipv6[IPV6_ADDRESS_LEN];
...
typedef struct gtp_trc_info_ie_t {
  ...
  uint32_t ip_addr_of_trc_coll_entity;
} gtp_trc_info_ie_t;
...
typedef struct gtp_pres_rptng_area_act_ie_t {
  ...
  tai_field_t tais[MAX_TAIS];
  macro_enb_id_fld_t macro_enb_ids[MAX_MACRO_ENB_IDS];
  home_enb_id_fld_t home_enb_ids[MAX_HOME_ENB_IDS];
  ecgi_field_t ecgis[MAX_ECGIS];
  rai_field_t rais[MAX_RAIS];
  sai_field_t sais[MAX_SAIS];
  cgi_field_t cgis[MAX_CGIS];
  uint8_t spare8 :2;
  uint8_t nbr_of_extnded_macro_enb :6;
  extnded_macro_enb_id_fld_t extnded_macro_enb_ids[MAX_EX_MACRO_ENB_IDS];
