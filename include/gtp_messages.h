...
typedef struct mod_bearer_cmd_t {
  gtpv2c_header_t header;
  gtp_agg_max_bit_rate_ie_t apn_ambr;
  gtp_mod_bearer_command__bearer_ctxt_ie_t bearer_context;
  gtp_ovrld_ctl_info_ie_t mmes4_sgsns_ovrld_ctl_info;
  gtp_ovrld_ctl_info_ie_t sgws_ovrld_ctl_info;
  gtp_ovrld_ctl_info_ie_t twanepdgs_ovrld_ctl_info;
  gtp_fully_qual_tunn_endpt_idnt_ie_t sender_fteid_ctl_plane;
...
typedef struct create_indir_data_fwdng_tunn_req_t {
  ...
  gtp_fully_qual_tunn_endpt_idnt_ie_t sender_fteid_ctl_plane;
  uint8_t bearer_count;
  gtp_create_indir_data_fwdng_tunn_request__bearer_ctxt_ie_t bearer_contexts[MAX_BEARERS];
  gtp_recovery_ie_t recovery;
  gtp_priv_ext_ie_t priv_ext;
...
typedef struct create_indir_data_fwdng_tunn_rsp_t {
  ...
  gtp_fully_qual_tunn_endpt_idnt_ie_t sender_fteid_ctl_plane;
  uint8_t bearer_count;
  gtp_create_indir_data_fwdng_tunn_response__bearer_ctxt_ie_t bearer_contexts[MAX_BEARERS];

  gtp_recovery_ie_t recovery;
  gtp_priv_ext_ie_t priv_ext;
} create_indir_data_fwdng_tunn_rsp_t;

typedef struct del_indir_data_fwdng_tunn_req_t {
	gtpv2c_header_t header;
}del_indir_data_fwdng_tunn_req_t;

typedef struct del_indir_data_fwdng_tunn_resp_t {
	gtpv2c_header_t header;
	gtp_cause_ie_t cause;
	gtp_recovery_ie_t recovery;
}del_indir_data_fwdng_tunn_resp_t;
...
typedef struct mod_acc_bearers_req_t {
	...
	gtp_fully_qual_tunn_endpt_idnt_ie_t sender_fteid_ctl_plane;
	gtp_delay_value_ie_t delay_dnlnk_pckt_notif_req;
	uint8_t bearer_modify_count;
	gtp_mod_bearer_request_bearer_ctxt_to_be_modified_ie_t bearer_contexts_to_be_modified[MAX_BEARERS];
	uint8_t bearer_remove_count;
	gtp_mod_bearer_request_bearer_ctxt_to_be_removed_ie_t bearer_contexts_to_be_removed[MAX_BEARERS];
	gtp_recovery_ie_t recovery;
	uint8_t second_rat_count;
	gtp_secdry_rat_usage_data_rpt_ie_t secdry_rat_usage_data_rpt[MAX_BEARERS];
	gtp_priv_ext_ie_t priv_ext;
...
typedef struct mod_acc_bearers_rsp_t {
	...
	gtp_mod_bearer_response_bearer_ctxt_modified_ie_t bearer_contexts_modified[MAX_BEARERS];
	uint8_t bearer_remove_count;
	gtp_mod_bearer_response_bearer_ctxt_marked_removal_ie_t bearer_contexts_marked_removal[MAX_BEARERS];
	gtp_recovery_ie_t recovery;
	gtp_indication_ie_t indctn_flgs;
	gtp_load_ctl_info_ie_t sgws_node_lvl_load_ctl_info;
	gtp_ovrld_ctl_info_ie_t sgws_ovrld_ctl_info;
...
typedef struct gtp_resume_notification_t {
	gtpv2c_header_t header;
	gtp_imsi_ie_t imsi;
	gtp_eps_bearer_id_ie_t linked_eps_bearer_id;
	gtp_node_type_ie_t originating_node;
	gtp_fully_qual_tunn_endpt_idnt_ie_t sender_fteid;
	gtp_priv_ext_ie_t priv_ext;
} gtp_resume_notification_t;

typedef struct gtp_resume_acknowledge_t {
	gtpv2c_header_t header;
	gtp_cause_ie_t cause;
	gtp_priv_ext_ie_t priv_ext;
...
/**
 * Description -Suspend notification message
 * */
typedef struct gtp_suspend_notification_t {
	gtpv2c_header_t header;
	gtp_imsi_ie_t imsi;
	gtp_eps_bearer_id_ie_t linked_eps_bearer_id;
	gtp_node_type_ie_t originating_node;
...
/**
 * Description -Suspend acknowledgement message
 * */
typedef struct gtp_suspend_acknowledgement_t {
	gtpv2c_header_t header;
	gtp_cause_ie_t cause;
	gtp_priv_ext_ie_t priv_ext;
} gtp_suspend_acknowledgement_t;
